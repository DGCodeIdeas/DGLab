...
phase_4() {
  ...
  # Detect any php*-fpm packages installed via apt/rpm
  ...
  if [[ ${#php_fpm_pkgs[@]} -gt 0 ]]; then
    ...
    if [[ $DRY_RUN -eq 0 ]]; then
      ...
      echo "[dry-run] systemctl stop/disable php*-fpm"
    fi
...
EOF
  ...
  if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -qi "active"; then
    ...
    if [[ $DRY_RUN -eq 0 ]]; then
      ...
      ufw status numbered 2>/dev/null | grep -i 'anvil' | awk '{print $1}' | sort -rn | while read -r num; do
        # Strip brackets from rule number
        num="${num//[[\]]/}"
        [[ "$num" =~ ^[0-9]+$ ]] && echo "y" | ufw delete "$num" 2>/dev/null || true
      done
    else
  ...
  if [[ $DRY_RUN -eq 0 ]]; then
    # System-level nssdb
    rm -rf /etc/pki/nssdb 2>/dev/null || true
