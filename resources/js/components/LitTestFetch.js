...
export class LitTestFetch extends LitElement {
    static properties = {
        data: { type: Object },
        status: { type: String }
    };
    ...
    async firstUpdated() {
        try {
            ...
            if (!response.ok) throw new Error('Offline or Error');
            this.data = await response.json();
            this.status = 'success';
            localStorage.setItem('lit-test-cache', JSON.stringify(this.data));
        } catch (e) {
            console.warn('Fetch failed, using cache', e);
            const cached = localStorage.getItem('lit-test-cache');
            if (cached) {
