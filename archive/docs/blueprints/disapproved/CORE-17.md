# CORE-17: Session & Cookie Manager
...
| Secure Random Generation | `/legacy/Architecture/CORE_FRAMEWORK.md` – *random_bytes()* | Use `random_bytes()` for generating cryptographically strong nonces and CSRF tokens (≥128‑bit entropy). |
...
| **CORE‑15 (Task Scheduler)** | May store task‑related session data for long‑running jobs; the scheduler can reuse the same session store. | Shared session space for async tasks. |
...
## SemVer Impact
**Minor** – Introduces a new session & cookie management layer without altering existing public APIs of CORE‑01 to CORE‑16. Existing applications can adopt the manager by registering it in the DI container; no breaking changes to core contracts. Breaking API changes (e.g., removing `SessionManager::start` or altering its return shape) would trigger a **Major** version bump.
