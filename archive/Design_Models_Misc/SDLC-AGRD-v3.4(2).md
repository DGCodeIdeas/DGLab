...
operationalizes the model above for the varied AI agents (any capability class — Local Editor, Local
CLI, Local Inline, Cloud PR, Cloud Async, Cloud Conversational; product names like Cline, Zoo, Kilo,
Aider, Jules, Devin, Claude web appear as illustrative examples only) the solo tech lead actually
drives. §11 is calibrated to v3.4's rules — when v3.5 lands (after lap 1 data, per §10), §11 needs
re-calibration too. v3.4 also references a separate MEMORY.md (at `/home/z/my-project/MEMORY.md`) as
the entry-point file every agent reads before any task; §11.3 step 0 makes this explicit.
...
*agents* the tech lead drives — classified by capability class (Local Editor, Local CLI, Local
Inline, Cloud PR, Cloud Async, Cloud Conversational — see §11.2) rather than by product name, since
new agents appear faster than any taxonomy can track. These agents multiply a solo engineer's implementation
...
This module is **agent-agnostic**. The rows below are **capability classes** — the requirement. The
"Examples" column lists product names (Cline, Zoo, Kilo, Aider, Jules, Devin, Claude web, Codex,
Copilot Workspace, etc.) purely as **illustrative, not prescriptive**. New agents — Kilo, Zoo
(formerly Roo Code), future tools not yet named — slot in by capability match, not by being listed
here. When a new agent appears, classify it by capability class and use the prompts that match that
class; do not wait for this table to be updated. The prompts in §§11.4–11.13 reference capability
classes (e.g., "any Local Editor Agent"), not product names, for exactly this reason.

Not all capability classes fit all tasks. Picking the wrong class for a task is a real failure mode
— a Cloud Async Agent on a lint-scope expansion (which requires deep iterative testing against
`run.php`) will produce something that looks plausible but doesn't actually catch drift; a Local
Editor Agent on a dependency-graph analysis (which benefits from a fresh-clone perspective and async
autonomy) will take longer than a Cloud Async Agent for the same output. The matrix below is the
calibration.

| Capability class | Examples (illustrative, not prescriptive) | Strengths in solo-mode DGLab | Use for | Do NOT use for |
|---|---|---|---|---|
| **Local Editor Agent** — VSCode-embedded, full repo context, iterative editor | Cline, Zoo (formerly Roo Code), Kilo, Continue.dev in deeper modes | Full repo context, iterative edits, runs CLI tools, reads specific files on demand | Bet execution (depth 1→5), lint scope expansion, ADR implementation, hand-off verification | Fresh-perspective review of v3.x docs (too close to repo) |
| **Local CLI Agent** — terminal, git-aware, atomic commits | Aider, GitHub Copilot CLI | Atomic commits with messages, conversational, strong on small-surface deepening | Deepening bets where many small commits matter, refactors within a single blueprint | Wide-surface widen steps (better with Local Editor's editor integration) |
...
| **Cloud Async Agent** — fresh-clone, longer-horizon, autonomous | Jules, Devin, Factory | Scoped task brief → PR, runs in fresh clone, more agency than Cloud PR class | Dependency analyses against `INDEX.md` §5.2, isolated implementation of well-defined lint checks, scaffold generation | Lint expansion (no iterative test loop), anything requiring worklog awareness mid-task |
| **Cloud Conversational Agent** — chat, no repo access | Claude web, ChatGPT/Codex, Gemini web | Gap-hunting review passes, OD analysis, ADR drafting, document review | §11.12 review passes, OD pre-resolution analysis, cross-document consistency checks, second-opinion on a PR | Direct repo edits without tech-lead review |

**Classification rule for a new agent:** match on capability, not on vendor. The two axes that matter
are (a) does the agent have full repo context (Local classes) or run against a fresh clone / no repo
(Cloud classes), and (b) does the agent operate iteratively with the tech lead in the loop (Local
Editor, Local CLI, Cloud Conversational) or autonomously to a PR (Cloud PR, Cloud Async). When in
doubt, classify conservatively — treat an unknown agent as Cloud Async (lowest context, highest
autonomy) until proven otherwise, because the hand-off bundle discipline (§11.13) is the safety net
for exactly that case.

**The cloud-vs-local split that matters most:** cloud agents (Cloud PR, Cloud Async, Cloud
Conversational) run against a fresh clone or no repo at all, with no worklog awareness and no
conversation history. They cannot see prior agent invocations. Local agents (Local Editor, Local
CLI, Local Inline) inherit the worklog and the in-editor state. **Any task handed to a cloud agent
...
(the project's curated entry-point file, at `/home/z/my-project/MEMORY.md`), read end-to-end; (b)
the **shared context block** below, with current-state fields filled in. Without both, the agent
will optimize for the locally visible goal and silently violate one of the global rules. With both,
the rules are in the agent's context window alongside the task.

**Step 0 — read MEMORY.md first.** This is non-negotiable, regardless of capability class. MEMORY.md
is the curated entry-point file: project identity, team reality, methodology pointer, the 7 rules
every agent must internalize, file map, vocabulary, current state snapshot, OD status, known failure
modes from the v3.x history, agent conventions, and the update protocol. Every agent reads it before
any task — Local Editor, Local CLI, Local Inline, Cloud PR, Cloud Async, Cloud Conversational, no
exceptions. If MEMORY.md is missing or stale, ASK before proceeding — its absence is a red flag, not
a permission to skip orientation. Cloud agents in particular: MEMORY.md must be in the hand-off
bundle (§11.13) since you have no other way to see it.
...
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered. Cooldown 0 has started;
Milestone 0 has not.
...
AGENT: any Local Editor Agent (§11.2) — e.g., Cline, Zoo, Kilo
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered. Cooldown 0 complete.
...
already-admitted blueprints in `INDEX.md` §5.2. This is a two-capability-class task by design: a
Cloud Async Agent for the dependency analysis (fresh-clone perspective, async autonomy, good at
structured graph queries), a Local Editor Agent for the actual admission commit (full repo context,
iterative editor). The hand-off between them is where §2.1 freeze discipline is most at risk — the
Cloud Async Agent doesn't see the worklog, the Local Editor Agent doesn't see the Cloud Async Agent's
analysis unless explicitly given it.

```
TASK: Lap <N> widen step — admit next-most-depended-upon Hub blueprint
AGENT: any Cloud Async Agent for dependency analysis; any Local Editor Agent for admission commit.
(§11.2)
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered to BOTH agents. Lap <N>
widen step active.
...
PHASE 1 — CLOUD ASYNC AGENT (dependency analysis):
...
7. Tech lead reviews the Cloud Async Agent's analysis. Produces hand-off bundle for the Local
   Editor Agent: recommended blueprint, in-scope files, out-of-scope files (frozen interfaces,
   dependents outside scope), §5.2 status.

PHASE 3 — LOCAL EDITOR AGENT (admission commit):
...
- §5.2 coverage status recorded (either "still has covered candidates" or "exhausted, §9
  trigger fired").
- Worklog updated by BOTH agents (Cloud Async Agent appends Phase 1 entry; Local Editor Agent
  appends Phase 3 entry).
...
- Skip the Cloud Async Agent phase and admit "the obvious next one." The whole point of the dependency
...
AGENT: any Local Editor Agent (iterative). Any Local CLI Agent acceptable for single-blueprint
refactors with many small commits. (§11.2)
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered. <BLUEPRINT-ID> is below
its per-blueprint floor for lap <N>.
...
Lint-scope expansion is explicitly **Local-Editor-Agent-only** — Cloud Async, Cloud PR, and Cloud
Conversational agents all lack the iterative test loop needed to actually validate that a new lint
check catches what it claims to catch. Don't be tempted to delegate this to a cloud agent to "save
the Local Editor Agent's time for bet work" — that trade produces lint checks that look plausible
and miss real drift.

```
TASK: Cooldown <N> — lint-scope expansion (one drift class)
AGENT: any Local Editor Agent. Do NOT use Cloud Async, Cloud PR, or Cloud Conversational agents
for lint expansion — they lack the iterative test loop needed to validate the check actually catches
drift. (§11.2)
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered. Cooldown <N> active
(NOT a bet week).
...
STEPS:
1. (Local Editor Agent) Diff the content tree against the contract tree since the last cooldown:
...
2. (Local Editor Agent) Output a drift report: matches, mismatches, orphans.
...
4. (Local Editor Agent) Apply the agreed changes. Update worklog.
...
AGENT: any Cloud Conversational Agent for ADR drafting; any Local Editor Agent for implementation
after approval. (§11.2)
...
STEPS:
1. (Local Editor Agent) STOP implementation. Do not edit the interface.
2. (Local Editor Agent) Document the specific deficiency: what couldn't be done at the target depth with
...
3. (Local Editor Agent) Capture current state in worklog: blueprints affected, depth reached before
   STOP, ADR request ID assigned.
4. (Cloud Conversational Agent) Draft the ADR using the project's ADR template. Include:
...
6. (Local Editor Agent) Only after approval: implement the change. Bump the interface version (per
   the project's versioning convention). Migrate dependents in the same commit (or a sequence of
   commits within the same bet — do not leave a half-migrated state at end of bet).
7. (Local Editor Agent) Update worklog with: ADR ID, dependents migrated, depth reached post-change,
...
AGENT: any Cloud Conversational Agent for analysis and ADR drafting; any Local Editor Agent for any
implementation after approval. (§11.2)
...
STEPS:
1. (Cloud Conversational Agent) Read OD-<NN>'s entry in Architecture/OPEN-DECISIONS.md. Read every
   cross-reference it cites.
2. (Cloud Conversational Agent) Draft the ADR. Include:
...
4. (Local Editor Agent) Implement per the ADR. Update OPEN-DECISIONS.md to mark OD-<NN> resolved
   with ADR ID and date.
5. (Local Editor Agent) Update worklog.
...
AGENT: any Local Editor Agent for state capture; tech lead for re-scoping decision. (§11.2)
...
STEPS:
1. (Local Editor Agent) STOP work immediately. Do not start new files. Do not start new test cases.
   Do not "just finish this one method."
2. (Local Editor Agent) Capture current state in a kill-event worklog entry:
...
3. (Local Editor Agent) Surface kill event to tech lead. Suggest re-scoping options:
...
5. (Local Editor Agent) Apply the decision. Begin next bet per the tech lead's scope.
...
AGENT: This is a tech-lead decision, NOT an agent decision. Agents surface; tech lead decides. Any
Local Editor Agent captures state; any Cloud Conversational Agent assists on reassessment analysis.
(§11.2)
TRIGGER: Milestone 0 has exceeded 8 weeks (§4). The Local Editor Agent surfaces this at week 7
(warning) and week 8 (hard stop).

STEPS:
1. (Local Editor Agent) At week 7 of Milestone 0: surface warning to tech lead. Include current
   state of all 8 blueprints (depth reached, blockers), end-to-end Pulse trace status.
2. (Local Editor Agent) At week 8 of Milestone 0: surface hard-stop. STOP all Milestone 0 work. Do
   not push to week 9.
3. (Local Editor Agent) Capture full state:
...
4. (Tech lead + Cloud Conversational Agent) Reassess. Three branches:
   - Branch A: architecture sound, solo throughput assumption wrong.
     → Options: bring on a second engineer; descope to a smaller Milestone 0 (fewer
       ...
       throughput improvements (better Local Editor Agent prompts, more Cloud Async Agent
delegation).
...
5. (Tech lead) Record the reassessment outcome in a new ADR. Do not silently continue
   with the same plan.
6. (Local Editor Agent) Apply the ADR's decision. Update worklog.
...
("the agent says it's almost done") or personal momentum override this. The kill trigger
...
AGENT: any Cloud Conversational Agent for the gap-hunt itself; tech lead for triage; any Local
Editor Agent for applying fixes. (§11.2)
PRECONDITION: §11.3 step 0 (MEMORY.md) and shared context block delivered. The document under
review is v<N>.
...
STEPS:
1. (Cloud Conversational Agent) Read the full document end-to-end. Read every cross-reference (every
   "§X.Y" citation). Open the cited section and verify the citation.
2. (Cloud Conversational Agent) For each cross-reference, verify:
...
3. (Cloud Conversational Agent) For each rule with a numeric parameter (depth levels, week counts,
   lap counts, multipliers like 1.5×), verify:
...
4. (Cloud Conversational Agent) For each "this document supersedes" claim, verify:
...
5. (Cloud Conversational Agent) For each "what this module does NOT promise" or "honest caveat"
   callout, verify:
...
6. (Cloud Conversational Agent) Output a numbered list of findings. For each:
...
8. (Local Editor Agent, after tech-lead decision) Apply accepted fixes. Produce v<N+1>. Update
   worklog.
...
Agent: <capability class (§11.2) + product name if known — e.g., "Local Editor Agent (Cline)"
or "Cloud Async Agent (Jules)" or "Local Editor Agent (Kilo)" or "Cloud Conversational Agent
(Claude web)">
...
Common scenarios:
- A Local Editor Agent starts a widen-step dependency analysis, hands off to a Cloud Async Agent
  for the structured graph query against INDEX.md §5.2 (Cloud Async is better at this — fresh
  clone, async).
- A Cloud Async Agent returns a candidate blueprint recommendation; tech lead hands off to a
  Local Editor Agent for the actual admission commit (Local Editor has full repo context, editor
  integration).
- A Local Editor Agent surfaces an interface-freeze violation; tech lead hands off to a Cloud
  Conversational Agent for ADR drafting (Cloud Conversational is better at structured prose with
  alternatives-considered).
- A Cloud Conversational Agent returns an ADR; tech lead hands off to a Local Editor Agent for
  implementation.
...
Cloud agents (Cloud Async, Cloud PR, Cloud Conversational classes) have less context than local
ones — they run against a fresh clone or no repo at all, with no conversation history and no
worklog awareness. The hand-off bundle is not optional politeness — it is the contract that keeps
cloud-agent work inside the §2.1 interface-freeze discipline. Skip it and you will find out at
integration time that a Cloud Async Agent edited a frozen interface "because it looked cleaner,"
which is exactly the kind of rework Spiral Deepening exists to prevent.
```
...
- **Agent prompts assume agent behavior that may drift.** All agents — local and cloud, current
  and future — update their behavior over time, and new agents appear (Kilo, Zoo, future tools not
  yet named). A prompt that works today may not work in three months — the agent may become more
  cautious (refusing tasks it would have done), more aggressive (editing files it would have left
  alone), or differently scoped (handling a different task surface). The tech lead is responsible
  for re-validating prompts against current agent behavior, not assuming the prompt is stable across
  agent versions. The §11.2 capability-class framing (rather than product names) is the mitigation:
  classify the agent by capability, use the matching prompt, and re-validate when behavior drifts.
