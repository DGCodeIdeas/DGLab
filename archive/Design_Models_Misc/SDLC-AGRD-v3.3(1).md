# SDLC-AGRD v3.3: Spiral Deepening for a Solo Tech Lead
...
than assuming it), and the "3 engineers" assumption embedded in all predecessors.
...
**Bet duration — stated, not left open.** A bet's time-box is the throughput-derived estimate for its
scoped work (§5), with a 1.5× ceiling as its own kill trigger, mirroring Milestone 0's own approach
(§4) rather than inheriting a fixed "6 weeks" from a 3-person predecessor with no basis in solo
velocity. Before Milestone 0 completes there is no real throughput number to derive this from — that's
expected; the first bet's time-box is set the moment Milestone 0's `W` is known, not before.

**Bet scoping rule:** pick the ring with the lowest average depth across its currently-touched
blueprints, and deepen it to the current lap's target floor (§4.3) before touching anything else.
Concretely, if `CORE-02`/`04`/`05`/`06` are sitting at depth 1–2 after Milestone 0 while
`BRIDGE-01`/`ISPOKE-09`/`ESPOKE-01` are at depth 2, Bet 1 is "Core to depth 3" — a named target ring
and depth, not a vague "keep deepening."
...
1. **Widen.** For each of the 6 rings, admit its next-most-depended-upon not-yet-touched blueprint at
   depth 1. "Next-most-depended-upon" is decided against the current matrix's real dependency edges
   (`INDEX.md` §5.2 — verified to be an actual Mermaid graph with real `A --> B` edges, not prose; see
   the coverage caveat under gap C below), not arbitrarily — e.g., once `HUB-01` is in, the next Hub
   admission is whichever other Hub blueprint the already-touched Bridge/Spoke set most needs (`HUB-02`
   Cache and `HUB-04` Identity are the likely first candidates, since
   `BRIDGE-01`/`ISPOKE-09`/`ESPOKE-01` all reference them even at low depth — both confirmed present
   with real edges in `§5.2`'s current coverage).
...
- **`INDEX.md` §5.2's dependency-graph coverage (37 of 96 blueprints, verified) needs extending before
  ...
  to compute against. Add that trigger (e.g., "extend §5.2 for a ring's remaining blueprints once its
  covered set is 50% admitted") during whichever cooldown first sees widening approach the 37-node
  boundary.
