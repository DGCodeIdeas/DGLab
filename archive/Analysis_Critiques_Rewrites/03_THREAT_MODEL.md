...
| Information disclosure | Hub service returns an internal entity (skips DTO transform). | Internal data shape | "Zero-Exposure Test" CI gate: no class in `Sovereign\External` may `use` anything from `Sovereign\Internal`. Static analysis enforces. | BRIDGE-01 |
...
- **Recommendation:** File ADR-011 to clarify that ES256 is **not** post-quantum; specify the migration path (Ed25519 or a hybrid ECDSA+dilithium scheme) as a future major SemVer change to CORE-16. Update CORE-16's description to remove the misleading claim until the migration lands.
...
- **Recommendation:** Add a static-analysis rule in CORE-20 Forge that rejects `->raw()`, `->statement()`, or `->unprepared()` calls in `Sovereign\Hub` and `Sovereign\Spoke` unless the call site carries a `#[BypassTenantScope]` attribute, gated to a small audited allowlist. Hard-fail in CI.
