...
## 5. Pulse Depth Control

The depth a Pulse penetrates is determined by **cache state** and **route design**:

```
Depth 0: CDN Cache (Outer Rim only)
    Trigger: Cache-Control: public, max-age > 0
    Cost: ~5ms
    Touch: None of the wheel

Depth 1: Hub Cache (Outer Spoke + Hub)
    Trigger: HUB-02 cache HIT
    Cost: ~15ms
    Touch: Rim → Spoke → Hub → return

Depth 2: Database Read (Outer Spoke + Hub + Core)
    Trigger: Cache MISS, read-only query
    Cost: ~50ms
    Touch: Full stack, no audit write

Depth 3: Database Write (Full stack + Audit)
    Trigger: Mutation endpoint
    Cost: ~100ms
    Touch: Full stack + HUB-06 audit + HUB-09 event

Depth 4: Deep Internal (Inner Spoke + Full stack)
    Trigger: Admin operation
    Cost: ~200ms
    Touch: Full stack + cross-spoke calls + heavy audit

Depth 5: System (Core + Hub only, no Rim)
    Trigger: Scheduled job, queue worker
    Cost: Variable
    Touch: Core + Hub + Deploy infrastructure
```

**Design goal:** 90% of Pulses should terminate at Depth 1 or shallower.
