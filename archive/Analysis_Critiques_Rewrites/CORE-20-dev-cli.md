...
## Resolves
- **Finding 2** — re-anchors CORE-20 to its verified identity per `01_MASTER_INDEX.md` §2: **Developer CLI Toolchain ("Sovereign Forge")**, namespace `SovereignStack\Forge`. The stale approved file (`docs/blueprints/Core/CORE-20.md`, 3 530 bytes) misnames the component "Sovereign Forge (DevTools)" and uses the wrong namespace `Sovereign\Core\Console\Commands`. This blueprint replaces both with the canonical name and PSR-4 mapping.
...
| `MakeHubCommand` | `final class extends Command` | `forge:make:hub <name> [--namespace=] [--no-register]`. Validates PascalCase name, scaffolds `packages/hub/<name>/` with `src/`, `tests/`, `composer.json`, `README.md`, a `ServiceProvider` skeleton with `#[AsProvider]`, and registers the new package in the monorepo `composer.json` `repositories` section unless `--no-register` is set. |
...
| `Scaffolder` | `final class implements ScaffolderInterface` | Orchestrates the make-* commands: validates the name, computes the target paths, calls `StubEngine` for each template, writes files via CORE-14 `FilesystemInterface`, and registers the new package via `MonorepoRegistry`. Tracks every file written so a failure mid-scaffold can be rolled back. |
| `MonorepoRegistry` | `final class implements MonorepoRegistryInterface` | Reads the monorepo root `composer.json`, appends a `repositories` entry of type `path` pointing at the new package, and writes it back atomically (temp file + `rename`). Detects duplicate registration and refuses to write. |
 ...
 * monorepo, and on any failure roll back the partial state.
 *
...
interface ScaffolderInterface
{
     ...
     *                                       templates to render, monorepo registration
     *                                       flag.
...
/**
 * Read/write access to the monorepo root composer.json `repositories`
 ...
 */
interface MonorepoRegistryInterface
{
    /**
     ...
     * @param string $packagePath Relative path from the monorepo root, e.g. "packages/hub/Cache".
...
use SovereignStack\Forge\MonorepoRegistry;
 ...
 * And registers the new package in the monorepo root composer.json
...
final class MakeHubCommand extends ForgeCommand
{
    ...
    public function configure(): void
    {
        ...
        $this->addOption(new InputOption(
            ...
            description: 'Do not register the new package in the monorepo composer.json',
        ));
    }
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($register) {
            $output->writeln('');
            $output->writeln(' Registered in monorepo: ' . $spec->packageName);
        }
        $output->writeln('');
...
/**
 * Atomic registrar for the monorepo root composer.json repositories
 ...
 */
final class MonorepoRegistry implements MonorepoRegistryInterface
{
    public function __construct(
        private readonly string $monorepoComposerJson = __DIR__ . '/../../../composer.json',
    ) {
    }

    public function register(string $packageName, string $packagePath): void
    {
        $raw = file_get_contents($this->monorepoComposerJson);
        if ($raw === false) {
            throw new \RuntimeException(sprintf('Could not read %s', $this->monorepoComposerJson));
        }
        ...
        $this->atomicWrite($this->monorepoComposerJson, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) . "\n");
    }
...
sequenceDiagram
    ...
    participant Reg as MonorepoRegistry
...
stateDiagram-v2
    ...
    Registering --> Failed: monorepo write conflict
    ...
    note right of RollingBack
        ...
        MonorepoRegistry write is atomic
...
**Downward (consumed by).** `bin/forge` is the developer's primary entry point. Every Hub, Internal Spoke, and External Spoke package's `composer.json` is initially produced by `forge:make:hub` / `forge:make:spoke`. CORE-01's `bin/loom` reads the `repositories` section that `MonorepoRegistry` maintains (Loom's `DependencyGraph` is populated from this list). DEPLOY-01's image build expects the package layout Forge produces (`src/`, `tests/`, `composer.json` with declared `SovereignStack\<Tier>\<Name>` PSR-4 mapping). Hub-tier management commands (HUB-01 `flags:list`, HUB-02 `cache:flush`, HUB-06 `audit:replay`) will be added to their respective packages via `forge:make:command` once that subcommand lands (post-1.0; see SemVer).
...
| Monorepo registry write atomicity | Harness: PHPUnit correctness test that simulates a `rename()` failure (by injecting a fake filesystem that throws on the second `rename` call), asserts the original `composer.json` is unchanged and the temp file is deleted. Baseline: not applicable (correctness, not latency). |
...
3. **Generated `composer.json` never includes secrets.** The `hub.composer.stub` template contains no `auth`, `config`, or `extra` keys that could leak environment values. The `MonorepoRegistry` write only adds a `repositories` entry — it does not copy or merge any environment-derived values.
...
5. **Monorepo registry writes are atomic.** `MonorepoRegistry::atomicWrite()` writes to a temp file and `rename()`s over the original. A crash mid-write never truncates `composer.json`. If `rename()` fails, the temp file is unlinked. The original is untouched.
