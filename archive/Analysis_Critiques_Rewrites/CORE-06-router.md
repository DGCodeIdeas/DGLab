...
| `RouteCompiler` | Pure transformer: `Route` → `CompiledRoute`. Converts `/users/{id}` to `^/users/(?P<id>[^/]+)$`, applies per-parameter constraints (`int` → `\d+`, `slug` → `[a-z0-9-]+`, `uuid` → `[0-9a-f-]{36}`). Throws `InvalidRoutePatternException` on malformed patterns. |
