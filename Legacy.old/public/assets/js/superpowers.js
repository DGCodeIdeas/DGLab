...
document.addEventListener('DOMContentLoaded', () => {
    ...
    Superpowers.morph = function(oldNode, html) {
        const temp = document.createElement('div');
        temp.innerHTML = html;
        const newNode = temp.firstElementChild;
        Superpowers.recursiveMorph(oldNode, newNode);
    };
    ...
    Superpowers.recursiveMorph = function(oldNode, newNode) {
        ...
        if (oldAttrs && newAttrs) {
            for (let i = oldAttrs.length - 1; i >= 0; i--) {
                if (!newNode.hasAttribute(oldAttrs[i].name)) oldNode.removeAttribute(oldAttrs[i].name);
            }
            for (let i = 0; i < newAttrs.length; i++) {
                if (oldNode.getAttribute(newAttrs[i].name) !== newAttrs[i].value) oldNode.setAttribute(newAttrs[i].name, newAttrs[i].value);
            }
        }
    ...
    Superpowers.handleAction = async function(el, action, event) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.changedPersisted && Object.keys(result.changedPersisted).length > 0) {
                    window.dispatchEvent(new CustomEvent('superpowers:state-changed', { detail: result.changedPersisted }));
                }
            }
        } catch (err) {
...
class SuperpowersDebugOverlay {
    ...
    initUI() {
        ...
        style.innerHTML = '#sp-debug-trigger { position: fixed; bottom: 20px; right: 20px; width: 50px; height: 50px; background: #6366f1; border-radius: 50%; color: white; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 99999; }';
