...
> application deployment. `DEPLOY-01` is now **Core & Hub Service Deployment**.
>
> **Content correction (Finding 18):** the served document root is the canonical `Architecture/` tree,
> **not** `docs/architecture/origin/` (Vision A, archived). Any `Dockerfile`/`render.yaml` derived from
> this blueprint must point at `Architecture/`.
