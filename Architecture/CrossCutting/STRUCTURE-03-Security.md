...
Every authenticated Pulse carries a JWT signed with **ES256** (ECDSA P-256 + SHA-256) per ADR-003.
The signing key is owned and held exclusively by **`HUB-04`'s `JwtService`**; `CORE-16` (Binary
Encryption Envelope) is used only to encrypt that key *at rest*, and `BRIDGE-01` never holds the
private key — it verifies through `HUB-04`. The `alg` header is pinned to `ES256` at verification
time; `none`, `HS256`, and `EdDSA`/`Ed25519` are rejected (RFC 8725 algorithm substitution).
This JWT is the **trust passport** that every inner layer inspects.

```php
// JWT Payload Structure (registered + custom claims)
{
  "sub": "01ARZ3NDEKTSV4RRFFQ69G5FAV",      // User ULID
  "tenant_id": "01ARZ3NDEKTSV4RRFFQ69G5FAV", // Tenant ULID (claim name matches HUB-04 `TokenClaims`; NOT `tid`)
