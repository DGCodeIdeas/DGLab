...
**ADR (Architecture Decision Record)** — A short Markdown document in `Architecture/ADRs/` recording a single architectural decision in Nygard format (Context, Decision, Status, Consequences). Mandated for any decision with a viable alternative (Master Index Rule 8). The accepted set is ADR-001..010; ADR-011 (HUB-31) is **proposed**.
...
**Dependency DAG** — The directed acyclic graph of cross-component dependencies defined in Master Index §5 and `hub-dependency-graph.md`. Cycles are a broken build (CI must fail) — including *mutual-downward* declarations, which `Verification/lint/run.php` detects. The only source for build ordering; ad-hoc ordering based on perceived urgency is forbidden.

**Deploy Tier** — The 5 deployment blueprints (`Deploy/DEPLOY-00..04`, all present): DEPLOY-00 docs site, DEPLOY-01 Core+Hub containers, DEPLOY-02 datastore provisioning, DEPLOY-03 Bridge + External Spokes with the Zero-Exposure Test, DEPLOY-04 dev→staging→prod promotion across 50+ repos.
...
### §1.0 Wheel-Model Vocabulary (normative)

Mirrors `CrossCutting/STRUCTURE-01-Wheel.md` Part D verbatim in substance. Before this consolidation
these terms were used across `STRUCTURE-01`–`STRUCTURE-09` and defined **nowhere**
(`Verification/INCONSISTENCIES.md` #9).
...
| **Rim** | A checkpoint ring. **Inner Rim** = `BRIDGE-01`; **Outer Rim** = `HUB-08`. These are the only sanctioned rim names — *Thick Rim* / *Thin Rim* are withdrawn. |
| **Spoke** | A member of the Inner Spoke or Outer Spoke layer. |
| **Thick / Thin** | A *property* of a spoke (breadth of unmediated Hub/Core access), never a ring name. A consequence of caller trust, not the classifier. |
| **Entity** | The originator of a Pulse: public entity, staff entity, or `system`. |
| **Entry radius** | Which gate an entity entered through — Outer Rim (`HUB-08`, public) or the Inner-Spoke staff gate (`ISPOKE-04`, staff). Fixed by entity class. |
| **Depth** | The deepest ring a Pulse's own routing path reaches, on the single scale below. The three competing scales in the v0.1/v0.2/v0.3 wheel documents are withdrawn. |
| **Traversal depth** | Depth measured *from the entry radius*; set by data/capability locality, not by the requester's authorization level. |
| **Pulse** | One unit of runtime work, described by the 6-tuple `(entity, entry_spoke, depth, exit_spoke, lane, pulse_class)`. **Reserved word** — no component may be named "Sovereign Pulse" except `HUB-15`. |
| **Lane** | The tenant scope of a Pulse: a tenant ULID, `system`, or `public`. |
| **Pulse class** | One of `live`, `dormant`, `purge`, `ignition`. |
| **Pulse type** | One of A (synchronous radial), B (asymmetric fan-out), C (origin-less / system-initiated). |

**Depth scale (the only sanctioned one):** 1 = Outer Rim · 2 = Outer Spoke · 3 = Inner Rim ·
4 = Inner Spoke · 5 = Hub · 6 = Core. A staff-entity Pulse enters at depth 4 and never has depths 1–3.
...
|---|---|---|---|---|
| HUB-01 | Sovereign Hub Config & Flags | — | Infra | Multi-tenant configuration and feature-flag service extending CORE-10; per-tenant overrides + probabilistic rollout. |
| HUB-02 | Sovereign Cache & State | — | Integ | Shared cache coordination with Cache Tags and Atomic Locks, extending CORE-15; Redis-backed (ADR-006). |
| HUB-03 | Sovereign Asset Engine | — | Infra | PHP-only asset pipeline: CSS/JS minification, fingerprinting, versioned manifests; eliminates Node.js. |
| HUB-04 | Sovereign Identity & Authentication | — | Sec | User lifecycle, sessions, Argon2id hashing, OAuth2/OIDC foundation. The only JWT issuer. |
...
| HUB-09 | Sovereign Signal (Event Bus) | — | Integ | Distributed pub/sub extending CORE-03 to cross-process delivery via Redis Streams. |
...
| HUB-15 | Sovereign Pulse (Health Check & Service Discovery) | — | Infra | Centralised health-monitoring and service-discovery registry for all Hub services. |
...
| HUB-31 | Real-Time Analytics & Metrics Ledger | — | Data | **PROPOSED, NOT ACCEPTED.** Real-time streaming business metrics for `ISPOKE-05`, `ISPOKE-12`, `ISPOKE-13`, `ESPOKE-05`, `ESPOKE-11`, `ESPOKE-14`. Distinct from `HUB-15` (service health) and `HUB-23` (async batch export). See `ADRs/ADR-011-hub-31-real-time-analytics.md` and `OPEN-DECISIONS.md`. Not counted in the tier total. |
...
| ID | Component | Status |
|---|---|---|
| ISPOKE-01 | Sovereign Command Center (Admin Panel) | Documented |
| ISPOKE-02 | Sovereign Forge Portal (Developer Portal) | Documented |
| ISPOKE-03 | Sovereign Health & Observability Dashboard (Health Dashboard) | Documented |
| ISPOKE-04 | Sovereign Staff Hub (Staff Identity — the staff entry gate) | Documented |
| ISPOKE-05 | Sovereign Insight (BI / Analytics) | Documented |
| ISPOKE-06 | Sovereign Shield (Moderation) | Documented |
| ISPOKE-07 | Sovereign Relay (Internal Notifications) | Documented |
| ISPOKE-08 | Sovereign Flow (Workflow) | Documented |
| ISPOKE-09 | Sovereign Codex (Knowledge Base) | Documented |
| ISPOKE-10 | Sovereign Compliance (Audit) | Documented |
| ISPOKE-11 | Sovereign Forge (Sandbox) | Documented |
| ISPOKE-12 | Sovereign Toggle (Feature Control) | Documented |
...
| ID | Component |
|---|---|
| ESPOKE-01 | Sovereign Canvas (CMS) |
| ESPOKE-02 | Sovereign Connect (API) |
| ESPOKE-03 | Sovereign Account (Auth) |
| ESPOKE-04 | Sovereign Discovery (Search) |
| ESPOKE-05 | Sovereign Growth (Marketing) |
| ESPOKE-06 | Sovereign Relay (External Notifications) |
| ESPOKE-07 | Sovereign Nexus (GraphQL API) |
| ESPOKE-08 | Sovereign Prism (Media Delivery) |
| ESPOKE-09 | Sovereign Market (Checkout) |
| ESPOKE-10 | Sovereign Pulse (Billing) — *soft collision, see `OPEN-DECISIONS.md`* |
| ESPOKE-11 | Sovereign Beacon (Support) |
| ESPOKE-12 | Sovereign Forge (Dev Portal) |
| ESPOKE-13 | Sovereign Bridgehead (Partner Gateway) |
| ESPOKE-14 | Sovereign Lens (Analytics) |
| ESPOKE-15 | Sovereign Sentinel (External Orchestrator) |

### §1.5 Bridge and Deploy Component Names
...
Quick reference for where things live. **`Architecture/` is the single source of truth**; every other
documentation tree listed below is archived and read-only (see the `ARCHIVED.md` in each).

| Path | Contents |
|---|---|
| `Architecture/INDEX.md` | Governance + the only ID→component numbering authority |
| `Architecture/AUTHORING_GUIDE.md` | The blueprint fidelity bar and canonical template |
...
| `Architecture/OPEN-DECISIONS.md` | Every still-unresolved fork; nothing is silently picked |
| `Architecture/Verification/` | `INCONSISTENCIES.md` + the CI lint (`lint/run.php`) |
