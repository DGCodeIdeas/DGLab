...
**Rollback procedure:** Each package is an independent PR; revert the failing one. Hub blueprints depending on a reverted package are blocked at entry. If the SuperPHP chain fails to compile, ADR-005 can be re-opened and Twig adopted (re-baseline required).
