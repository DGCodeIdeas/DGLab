...
| `DbHealthChecker` | `final class implements HealthCheckerInterface` | Probes a database connection via CORE-19. `check(): HealthStatus` runs `SELECT 1` (or `SELECT 1 on MySQL) against a `ConnectionInterface` and asserts a response within `timeout_ms`. Returns `healthy` on success, `unhealthy` on any `\Throwable`. Used inside each Hub service's own `/health` endpoint, not directly by `HealthService` (the central aggregator only does HTTP). |
