...
### SQL DDL
MySQL 8 (InnoDB) per ADR-013. ULID primary keys per ADR-009. Argon2id hashes stored as `TEXT` per ADR-008 (self-describing `$argon2id$…` format).
...
| Login end-to-end (`authenticate()` incl. DB + Argon2id + JWT issue + Redis SET) | PHPUnit integration test, 100 iterations | Same + MySQL 8 on `localhost` | 1 thread, 100 cycles | ~150 ms/login — provisional, unverified |
