...
| `RouteCompiler` | Pure transformer: `Route` → `CompiledRoute`. Converts `/users/{id}` to `^/users/(?P<id>[^/]+)$`, applies per-parameter constraints (`int` → `\d+`, `slug` → `[a-z0-9-]+`, `ulid` → `[0-9A-HJKMNP-TV-Z]{26}`). Throws `InvalidRoutePatternException` on malformed patterns. |
