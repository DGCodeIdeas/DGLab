...
| `MakeSpokeCommand` | `final class extends Command` | `forge:make:spoke <name> --tier=internal|external`. Mirrors `MakeHubCommand` but writes to `app/Spokes/<Internal|External>/<Name>/` and selects the appropriate ServiceProvider template (internal spokes use `#[AsProvider]`; external spokes add a `#[BridgeRouted]` attribute). |
