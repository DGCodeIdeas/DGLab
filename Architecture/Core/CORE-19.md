...
- **Type mapping test (round-trip):** Insert a row with `created_at = new \DateTimeImmutable('2026-08-04T12:34:56.789+02:00')`. Fetch the row back. Assert the fetched `created_at` (via `TypeMapper::fromSql('timestamp', $value)`) represents the same instant as the original (`$fetched->getTimestamp() === $original->getTimestamp()`) to microsecond precision. MySQL's `TIMESTAMP` normalizes to the connection's session timezone rather than preserving the original offset the way PostgreSQL's `timestamptz` does (a capability gated behind `DriverInterface::supports('timezone_offset')`, false on the default MySQL driver) — offset preservation is not asserted here, only instant-equality. Repeat with `bool` (true / false), `null`, `int`, `float`, ULID string.
