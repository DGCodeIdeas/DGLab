...
8. ~~**No `version:release` composite command.** The operator must run `version:bump` (preview), read the output, then run `tag:create <repo> <computed-version>`. Automation wants one command that does both atomically. Fix: add `loom version:release <repo>` that calls `analyze()` → `calculateNewVersion()` → `tag()` → `pushTag()` in sequence, with a `--dry-run` flag for previews.~~ **RESOLVED 2026-08-16 (P2 PR)** — New `bin/loom version:release <package>` composite command runs the full release sequence in one invocation: (1) compute bump via shared `computeVersionBump()` helper, (2) compare against `composer.json` `version` field — if equal, exit with `action: skipped`; (3) if `--dry-run`, print plan and exit; (4) assert clean tree (passes `--allow-untracked` entries through to `assertClean()`); (5) gate CI via `--require-ci-green` (reuses `CiGate` from P1 gap 6); (6) bump `composer.json` via `Manifest::setVersion()`; (7) commit via `RepoManager::commitFile()`; (8) create annotated tag; (9) push tag to `--remote` URL if given. Supports `--message`, `--no-push`, `--remote <url>`, `--format=json`, plus all `--ci-*` passthrough flags.

9. ~~**No JSON output.** `version:bump` prints a human-readable table. The release workflow has to grep the output. Fix: add `--format=json` to `version:bump` and `version:release`.~~ **RESOLVED 2026-08-16 (P2 PR)** — Both `loom version:bump` and `loom version:release` accept `--format=json` (and the `--format json` space-separated form). The JSON payload includes `package`, `path`, `tag_prefix`, `current_version`, `new_version`, `increment`, `reason`, `commits_analyzed`; `version:release` additionally emits `action` (`released` / `skipped` / `dry_run`), `tag_name`, `tag_pushed`, `composer_json_updated`, and `commit_sha`. The release workflow's `Compute next version` step now uses `jq -r '.new_version'` instead of the previous `sed -nE 's/^New version:\s+.../p'` scraping.

10. ~~**`composer.json` `version` field drift.** After `tag:create`, the package's `composer.json` `"version"` field is stale. Fix: `version:release` should bump the field in the same commit (open file → JSON-decode → bump → JSON-encode → write → `git commit packages/<name>/composer.json -m "chore(<name>): bump version to X.Y.Z"` → then tag the bump commit). Alternatively, document that the tag is canonical and the field is informational-only (current policy, but not stated in any blueprint).~~ **RESOLVED 2026-08-16 (P2 PR)** — New `SovereignStack\Orchestrator\Manifest` class with static `setVersion(string $composerJsonPath, string $version): void` and `getVersion(string $composerJsonPath): string`. Writes atomically (write to `.loom-tmp` sibling, rename) so a partial write never corrupts the manifest. Preserves key ordering via `JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE`. New `RepoManager::commitFile(string $path, string $message): string` runs `git add <path>` + `git commit -m <message>` and returns the new HEAD SHA (40-char hex, validated). 13 new tests in `ManifestTest` cover happy path, field preservation, overwrite, invalid SemVer, missing file, invalid JSON, temp-file cleanup, trailing newline, and both `getVersion` branches. 3 new tests in `RepoManagerTest` cover SHA return, path-only staging, and failure on nothing-to-commit.

11. ~~**`repos.json` does not exist.** `loadConfig()` falls back to a default containing only `orchestrator`. The release workflow cannot iterate packages without a config. Fix: generate `orchestrator/repos.json` from `packages/*/composer.json` at workflow start (a 10-line PHP script), OR replace the file-based config with directory scanning.~~ **RESOLVED 2026-08-16 (P2 PR)** — New `bin/loom repos:generate [--output <path>]` command scans `packages/*/*/composer.json` via `MonorepoPackage::discover()` and writes `orchestrator/repos.json` (path overridable). Each entry has `name`, `ci_url` (absolute path), `token` (empty), `tier`, `tag_prefix` (`<tier>-<short>-v`), and `depends_on` (empty array). Replaces the previous 18-line inline `php -r '...'` script in `release.yml`.

### Reference workflow

The file `.github/workflows/release.yml` demonstrates the target state. It is **gated on `LOOM_RELEASE_ENABLED=1`** (default off) so it can land in `main` without taking action until the remaining gap (P1-5 branch protection) is closed. The workflow:

- Triggers on push to `main` (after the existing `packages-ci.yml` passes) or via `workflow_dispatch` with a `dry_run` input (default `true`).
- Iterates the library packages (`core/container`, `core/event-dispatcher`) via the matrix strategy.
- Generates `orchestrator/repos.json` via `loom repos:generate` (P2 gap 11 closed) — replaces the previous inline `php -r '...'` script.
- Verifies `bin/loom` is executable (Finding 21 guard).
- Gates on clean working tree via `loom status:clean --allow-untracked orchestrator/repos.json` (P1 gap 7 closed).
- For each package, runs `loom version:bump <package> --format=json` and parses the result with `jq -r '.new_version'` (P0 gaps 1, 3 closed — prefix and path scope handled by `MonorepoPackage`; P2 gap 9 closed — JSON output replaces sed scraping).
- Compares against the package's current `composer.json` `version` field; if equal, skips (no changes).
- If different, runs `loom version:release <package> --message <msg> --remote <token-url> --require-ci-green --ci-workflow packages-ci.yml --ci-branch main --ci-repo ${{ github.repository }} --ci-timeout 60 --allow-untracked orchestrator/repos.json` (P2 gaps 8, 10 closed — loom handles composer.json bump + commit + tag + push atomically; P1 gap 6 closed — `--require-ci-green` queries the GitHub Actions API inline and refuses to release a red main). The workflow then pushes the bump commit to `main` separately via `git push <token-url> main` and restores the original remote URL (PAT hygiene).
- Creates a GitHub Release with auto-generated notes via `gh release create`.

No inline workarounds remain — every step delegates to a loom command.
...
4. ~~Close P2 gaps 8–11 in a fourth PR: add `version:release` composite command, `--format=json`, `composer.json` field sync, generate `repos.json` from `packages/*/composer.json`.~~ **DONE 2026-08-16 (P2 PR)** — New `Manifest` class + `RepoManager::commitFile()` + `bin/loom version:release` + `bin/loom repos:generate` + `--format=json` on `version:bump` and `version:release`. 16 new tests (ManifestTest + RepoManagerTest extensions). Release workflow rewritten to use the new commands; no inline workarounds remain.
5. Flip `LOOM_RELEASE_ENABLED=1` in the repository variables. The workflow takes over from there. (Requires also creating the `LOOM_RELEASE_PAT` secret with `contents: write` scope, and configuring P1 gap 5 — Conventional-Commit PR title enforcement — on the repo Settings → Branches page.)

All code gaps are now closed. The only remaining gap is P1-5 (Conventional-Commit PR title enforcement), which is a GitHub branch-protection configuration, not loom code. Once that is configured and `LOOM_RELEASE_ENABLED=1` is flipped, the workflow takes over from manual tagging: a push to `main` triggers `loom version:release` for each package with path-scoped commits, and the loom handles prefix-aware tag naming, composer.json sync, annotated-tag creation, and PAT-hygiene push automatically.
