...
| 2026-09-23 | **Silent no-op trilogy + end-to-end release workflow fix + doctrine §4.5.3 + §4.5.4 implementation** (PRs #244-#251). The release workflow's `Tier versions: not yet released` display issue (asked about many monorepo releases ago) is **fully resolved** for the core tier. The 4-PR silent no-op series: (1) PR #244 `VersionBumpEngine::calculateNewVersion` 4-segment regex (Claude's diagnosis — necessary but not sufficient); (2) PR #245 `loom` `dirname(__DIR__, 2)` + composer.json fallback for `getCurrentVersion()` sentinel (real root cause #1); (3) PR #247 `RepoManager::getLogSince` no-tag case (root cause #3); (4) PR #248 PSR-18 client install + TOKEN_URL sed fix + `CIMonitor::check` local-path routing (final blockers). End-to-end verification: triggered `workflow_dispatch` run #82 → 5 bump commits landed on `main` + 7 per-PACKAGE tags created (`core-config-v0.2.0.0`, `core-container-v0.2.0.0`, `core-dbal-v0.2.0.0`, `core-kernel-v0.2.0.0`, `core-logger-v0.2.0.0`, `core-middleware-v0.2.0.0`, `core-router-v0.2.0.0`) + 3rd per-TIER tag `core-v0.2.0.0+b4e7e66` correctly referenced by monorepo release `v1.2.10.0+13828a1`. Doctrine §4.5.3 (item #2 of 6) implemented in PR #249: `Kernel::BOOTSTRAPPER_TIMEOUT_SECONDS = 5.0` frozen constant + `Kernel::$bootstrapperTimeoutSeconds` protected property + `KernelException::bootstrapperTimeoutExceeded()` named constructor + `microtime()` wrap in boot loop + 2 tests. Doctrine §4.5.4 (item #3 of 6) implemented in PR #251: `PanicException` class (extends `\RuntimeException`, class Panic per §2) with 5 named constructors + 4 invariant-violation throw-points in `Kernel.php` (factory null check, pipeline null check, state-recovery gap, releaseReferences unexpected-null) + catch-block skip on PanicException + 5 tests. Root cause #4 (`Manifest::setVersion` 3-segment regex — same bug pattern as PR #244, different throw site) fixed in PR #250. Composer.json constraints updated from `^0.1` to `^0.1 || ^0.2 || ^0.3` across root + 7 per-package files to allow the bumped package versions. | Release workflow end-to-end + doctrine §4.5.3 + §4.5.4 |
| 2026-09-20 | **Nuclear-Grade Engineering Doctrine** published at `CrossCutting/NUCLEAR-GRADE-DOCTRINE.md`. Binding on Step 5 Core persistence packages (CORE-19 DBAL, CORE-15 Cache, CORE-14 Filesystem, CORE-16 Encryption). Raises the depth-2 bar from "interfaces + happy-path tests" to "interfaces + chaos + audit + breaker + zeroization + property + concurrency + constant-time tests". Defines 12 binding principles, 5-class error taxonomy (Transient / Permanent-External / Permanent-Local / Corrupt / Panic), hard resource ceilings per package, panic procedure, audit hash-chain schema, and merge acceptance gate (§9). Where this doctrine conflicts with a softer statement in a per-package blueprint, the doctrine wins. Cross-referenced from `INDEX.md` §1, `Architecture/README.md`, root `README.md`, `Core/CORE-{14,15,16,19}.md`, `CrossCutting/THREAT_MODEL.md`, `CrossCutting/OBSERVABILITY.md`, `CrossCutting/STRUCTURE-03-Security.md`, `CrossCutting/STRUCTURE-05-Persistence.md`, `FROZEN-CONTRACTS.md`, `AUTHORING_GUIDE.md` | Nuclear-grade doctrine |
| 2026-09-23 | **Doctrine scope widened from "Step 5 Core Persistence Layer" to "Core Tier"** — all Core packages now bound in principle under §1–§3 and §5–§11 immediately, with per-package application sections §4.6 onward landing incrementally per §11's amendment protocol. **§4.5 CORE-18 — Kernel** added as the pilot for the broader scope: state-machine invariants (frozen), the four untested re-entrancy exceptions (`bootDuringBoot`, `handleDuringBoot`, `terminateDuringBoot`, `terminateDuringHandling`) flagged for immediate P11 closure (verified 2026-09-23 — `KernelStateMachineTest.php` lines 19, 21, 24, 25 list them but ship zero actual test methods), bootstrapper chain circuit breaker (P6 — 5s per-bootstrapper wall-clock + 30s outer watchdog + `BootstrapperTimeoutExceeded`), new `PanicException` class for invariant violations (§6 — null factory result, `releaseReferences()` failure, `assertBooted()` passing but `$pipeline` null, `handle()` finally cannot restore state), resource ceilings (30s boot / 30s handle / 5s terminate / 32 bootstrapper hard cap), `KernelLifecycleRecord` audit feed (8 record types via existing event dispatch), 8 worst-case chaos scenarios. `CORE-18.md` blueprint amended with doctrine cross-reference appendix per `AUTHORING_GUIDE.md` Step 5 rule. Motivation: external review (Claude, trace_id 1a0ca7fa404300c1) — the doctrine's own P1/P11/§11 imply the scope cannot be limited to the persistence layer; a Kernel panic or Container runaway is just as system-sinking as a corrupt cache payload. §11.1 amendment log records both amendments. | Doctrine scope-widening + CORE-18 pilot |
| 2026-09-24 | **Doctrine §4.5 CORE-18 Kernel pilot FULLY IMPLEMENTED** (PRs #253-#258). All 6 items complete: (1) §4.5.2 re-entrancy tests (#246), (2) §4.5.3 bootstrapper circuit breaker (#249), (3) §4.5.4 PanicException + 4 throw-points (#251), (4) §4.5.5 resource ceilings — 30s boot/30s handle/5s terminate/32 bootstrapper cap (#253), (5) §4.5.6 KernelLifecycleRecord audit feed — 7 lifecycle points + getLifecycleRecords() (#256), (6) §4.5.7 8 chaos tests — all scenarios covered (#257). Plus: runaway release loop structural fix — getLogSince no-tag fallback changed from ALL commits to empty array (#255), per-package tag health-check job added to release.yml (#258), 10 baseline per-package tags created at bc5affc, 8 orphaned releases + 8 orphaned tags deleted, 5 old v0.1.5.0-v0.1.9.0 orphaned releases deleted. Composer constraints relaxed to `*` with `composer validate` (no `--strict`). | Doctrine §4.5 complete + release workflow stabilized |
