...
function app(): Application { return Application::getInstance(); }

function config(string|array $key, mixed $default = null): mixed
{
    if (is_array($key)) {
        foreach ($key as $k => $v) Application::getInstance()->setConfig($k, $v);
        return null;
    }
...
function json(array $data, int $status = 200): Response { return app()->get(ResponseFactoryInterface::class)->json($data, $status); }
function redirect(string $url, int $status = 302): Response { return app()->get(ResponseFactoryInterface::class)->redirect($url, $status); }
function route(string $name, array $parameters = []): string { return app()->get(\DGLab\Core\Router::class)->url($name, $parameters); }
...
function asset(string $path): string
{
    ...
    if (file_exists($manifestPath)) {
        ...
        if (isset($manifest[$path])) return '/assets/' . $manifest[$path];
    }
...
function str_random(int $length = 16): string { return bin2hex(random_bytes($length / 2)); }
...
function env(string $key, mixed $default = null): mixed
{
    ...
    if (preg_match('/^"(.*)"$/', $value, $matches)) return $matches[1];
    return $value;
}
...
if (!function_exists('dd')) {
    function dd(...$args): never { foreach ($args as $arg) var_dump($arg); die(); }
}

if (!function_exists('dump')) {
    function dump(...$args): void { foreach ($args as $arg) var_dump($arg); }
}

function is_ajax(): bool { return ($_SERVER['HTTP_X_REQUESTED_WITH'] ?? '') === 'XMLHttpRequest'; }
...
function clear_cache(?string $pattern = null): void
{
    ...
    foreach (glob($cachePath . '/*.cache') as $file) {
        if ($pattern === null || strpos(basename($file), $pattern) !== false) @unlink($file);
    }
}
...
{
    if (is_string($event)) $event = new \DGLab\Core\GenericEvent($event, $payload);
    \DGLab\Facades\Event::dispatch($event);
}

function auth(): \DGLab\Services\Auth\AuthManager { return Application::getInstance()->get(\DGLab\Services\Auth\AuthManager::class); }
function mangascript(): MangaScriptService { return Application::getInstance()->get(MangaScriptService::class); }
